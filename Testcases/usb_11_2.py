...
def hibernate_during_copy():
    ...
    logger.info(f"{'*' * 20}TestCase started {'|usb_11.2|Hibernate During Copy|'}{'*' * 20}")
    usb_reconnect()
    copy_hibernate()
    ...
    try:
        ...
        error_msg = f"{test_file_name} file copy process not able to resume after Suspended"
        logger.error(f"{test_file_name} file copy process not able to resume after Suspended")
        print(red("Fail"))
        logger.error("Fail")
        ...
        logger.info(f"{test_file_name} file copy process able to resume after Suspended")
        print(green("PASS"))
        logger.info("PASS")
        return 'PASS', None
        ...
        logger.info(f"{'*' * 20}TestCase ended {'|usb_11.2|Hibernate During Copy|'}{'*' * 20}\n")
