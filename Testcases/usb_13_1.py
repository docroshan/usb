...
def bootable_usb():
    ...
    logger.info(f"{'*'*20}TestCase started {'usb_13.1'}{'*'*20}")
    path = devpath()
    exec_command(f"umount {dev}")
    print(f"Installing IOS image in {dev} path")
    logger.info(f"Installing IOS image in {dev} path")
    ...
    logger.info(f"{'*'*20}TestCase ended {'usb_13.1'}{'*'*20}\n")
