...
@fun_count
def shutdown_during_copy():
    print(f"{'*' * 20}TestCase started {blue('|usb_11.4|Shutdown Copy|')}{'*' * 20}")
    logger.info(f"{'*' * 20}TestCase started {'|usb_11.4|Shutdown Copy|'}{'*' * 20}")
    usb_reconnect()
    power_event("sudo rtcwake -s 60 -m off\n")
    ...
    try:
        ...
        close()
        print(f"{'*' * 20}TestCase ended {blue('|usb_11.4|Shutdown Copy|')}{'*' * 20}\n")
        logger.info(f"{'*' * 20}TestCase ended {'|usb_11.4|Shutdown Copy|'}{'*' * 20}\n")


shutdown_during_copy()
