...
def drive_partition():
    ...
    logging.info(f"Total USB Disk size: {res}")
    ...
    if (int(partitions_number)*int(result[0])) <= float(res):
        ...
        logging.info(f"Creating {partitions_number} Partitions of {partition_size} each.\n")
        ...
        logging.error(f"Memory Exceeding, Cannot create {partitions_number} Partitions of {partition_size} each.")
